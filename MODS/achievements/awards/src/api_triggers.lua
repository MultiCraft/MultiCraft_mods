	...
	if tdef.type == "counted" then
		...
		function tdef:on_register(def) -- luacheck: ignore
			...
			function def.get_progress(_, data)
				local current = min(data[tname] or 0, tmp.target)
				return {
					current = current,
					target = tmp.target,
			...
			function def.getDefaultDescription(_)
				...
				return tdef.auto_description[1]:format(n), tdef.auto_description[2]:format(n)
			end
	...
	elseif tdef.type == "counted_key" then
		...
		function tdef:on_register(def) -- luacheck: ignore
			...
			function def.getDefaultDescription(_)
				...
				if tmp.key then
					local nname = tmp.key
					return tdef.auto_description[1]:format(n),
							tdef.auto_description[2]:format(n, nname)
				else
					return tdef.auto_description_total[1]:format(n),
							tdef.auto_description_total[2]:format(n)
				end
			end
...
function awards.register_on_unlock(func)
	awards.on_unlock[#awards.on_unlock + 1] = func
end
