...
function mob_class:item_drop()
	...
	for n = 1, #self.drops do
		local drop = self.drops[n]
		if random(drop.chance or 1) == 1 then
			num = random(drop.min or 1, drop.max or 1)
			item = drop.name
...
function mob_class:do_jump()
	...
	and not blocked and not self.facing_fence and nod.name ~= node_snow then
			...
			minetest.after(0.3, function(self, v)
				if self.object:get_luaentity() and self.order ~= "stand" and
						self.order ~= "sit" then
...
function mob_class:falling(pos)
	...
	elseif v.y <= 0 and v.y > fall_speed then
	...
	and self.object ~= hitter then
		...
		for n = 1, #objs do
			...
			if obj then
				...
				-- creatura support
				if obj._creatura_mob then
					if (obj.owner or "") == name and obj.owner_loyal then
						obj._target = self.object
					end
				end
			end
		end
...
--		print("standing in " .. self.standing_in)
		...
		local standing_in = minetest.registered_nodes[self.standing_in]
		if standing_in.walkable
		and standing_in.drawtype == "normal" then
			self.object:set_velocity({
				x = 0,
				y = self.jump_height,
				z = 0
...
local count_mobs = function(pos, def)
	local mob_name, mob_type = def.name, def.mob_type
	local total, players = 0, false
	...
	for _, obj in ipairs(objs) do
		if obj:is_player() then
			...
			local ent = obj:get_luaentity()
			if ent then
				local ent_type = ent.mob_type
				if mob_type and ent_type and mob_type == ent_type then
					total = total + 1
				elseif ent.name and ent.name == mob_name then
					total = total + 1
				end
			end
		end
...
function mobs.spawn_action(def, pos, node, active_object_count, active_object_count_wider)
	...
	local num_mob, is_pla = count_mobs(pos, def)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "mobs_nametag"
		...
		if fields.name == "{remove_owner}" then
			...
			-- limit name entered to 32 characters long
			fields.name = sub8(fields.name, 1, 32)
